...
def coveragePerUtr(utrBed, bam, minQual, outputCSV, outputPDF, log, printOnly=False, verbose=True, force=True):
        
    if(not checkStep([bam], [outputCSV], force)):
        print("Skipped computing T->C per UTR position for file " + bam, file=log)
    else:
        coverageWindow = 200
        meanUtrCoverage = numpy.array([0] * coverageWindow)
        meanNormUtrCoverage = numpy.array([0] * 100)
        utrCount = 0
    
        for utr in BedIterator(utrBed):
            region = utr.chromosome + ":" + str(utr.start) + "-" + str(utr.stop)
            utrLength = utr.stop - utr.start
            
            utrCoverage = numpy.array([0] * coverageWindow)
            readNumber = 0        
            ...
            for read in bamFile.fetch(region=region):
                ...
                readNumber += 1
                alignedPos = numpy.array(read.get_reference_positions())
                
                #print(utr.name, utrLength)
                #print(read.query_name)
                #print(alignedPos)
                alignedPos = alignedPos - utr.start
                #print(alignedPos)
                alignedPos = utrLength - alignedPos
                #print(alignedPos)
        
                for pos in alignedPos:
                    if(pos < coverageWindow):
                        utrCoverage[pos] += 1
                        hasCoverage = True
                        
                    pos = int(pos * 100.0 / utrLength)
                    meanNormUtrCoverage[pos] += 1
        
            if(hasCoverage): 
                utrCount += 1
                print(utr.name, utr.start, utrLength, file=sys.stderr)               
#                 print(utr.chromosome + ":" + str(max(0, utr.stop - 200)) + "-" + str(utr.stop))
#                 print(utrCoverage)
                utrCoverage = utrCoverage * 100.0 / readNumber
#                 print(utrCoverage)
                meanUtrCoverage += utrCoverage
                
            if(utrCount > 1000):
                break
        
        meanUtrCoverage = meanUtrCoverage * 1.0 / utrCount
        print(meanUtrCoverage, file=sys.stderr)
        i = len(meanUtrCoverage) - 1
        while(i > -1):
            print(coverageWindow - i, meanUtrCoverage[i], sep="\t")
            i -= 1
                
#                 getchar()
                    
                #print((alignedPos - utr.start) * 100.0 / utrLength)
...
#         
#         #Go through one utr after the other
#         testFile = SlamSeqBamFile(bam, referenceFile)
#         
#         for utr in BedIterator(utrBed):
#                                          
#             readIterator = testFile.readInRegion(utr.chromosome, utr.start, utr.stop, maxReadLength)
